...
namespace GameLibrary
{
	...
	bool Dictionary::loadFromData(const Data& data, String* error)
	{
...
// plist parse functions
	...
	void Dictionary_base64Encode(std::string&dataEncoded, const Data& data)
	{
	...
	bool Dictionary_parse(const void*ptr, pugi::xml_node&node, Any&any, String*error)
	{
		...
		else if(type.equals("data"))
		{
			...
			any = Data((const void*)data_vect.data(), data_vect.size());
			return true;
		}
...
// plist write functions
	...
	bool Dictionary_write(pugi::xml_node&node, const Any&any, String*error)
	{
		...
		else if(any.is<Data>())
		{
			...
			const Data& data = any.as<Data>(false);
		...
		else if(any.is<Number>())
		{
			...
			if(number.isBool())
			{
				bool val = number.template toArithmeticValue<bool>();
				if(val)
				{
		...
		else if(any.is<ArrayList<Data> >())
		{
			const ArrayList<Data>& list = any.as<ArrayList<Data> >(false);
			return Dictionary_writeArray(node, list, error);
		}
	...
	void Dictionary_toJSON(const Any&any, String*output)
	{
		...
		{
			Dictionary_toJSON(any.as<Data>(false).toString(), output);
		}
		else if(any.is<DateTime>())
		{
		...
		{
			Dictionary_toJSON(any.as<ArrayList<Data> >(false), output);
		}
		else if(any.is<ArrayList<GameLibrary::Int64> >())
		{
